...
void SkimTree_LRSMHighPt::executeEvent(){
  ...
  if(!IsDATA){
    ...
    for(unsigned int i = 0; i < muons_all.size(); i++){

      Gen matched_gen = GetGenMatchedLepton(muons_all.at(i), gens);

      double current_muon_matched_pt = -1;
      double current_muon_matched_eta = -1;
